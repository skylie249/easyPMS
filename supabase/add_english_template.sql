-- English checklist template support
-- Adds a `locale` column to the template tables (existing rows default to 'ko',
-- so nothing about the current Korean template changes) and inserts the
-- English template content so /en/new can query locale = 'en'.
--
-- 적용 방법: Supabase 대시보드 > SQL Editor에서 이 파일 전체를 실행하세요.
-- 이미 실행한 경우 다시 실행해도 영어 템플릿이 중복 삽입되지 않습니다.

alter table template_categories add column if not exists locale text not null default 'ko';
alter table template_items add column if not exists locale text not null default 'ko';
...
declare
  ...
  cat_risk uuid;
begin
  if exists (select 1 from template_categories where locale = 'en') then
    raise notice 'English template already seeded, skipping.';
    return;
  end if;
...
-- 확인용:
-- select locale, count(*) from template_categories group by locale;
-- select locale, count(*) from template_items group by locale;
